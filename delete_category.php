...
$datosRecibidos = isset($_SESSION['id']);
if ($datosRecibidos) {
    ...
    if ($permiso > 0) {
        $idAEliminar = $_GET['id_categoria'];
        $eliminarFila = "DELETE FROM categoria WHERE id_categoria = $idAEliminar";
        $eliminarEnDB = mysqli_query($conexion, $eliminarFila);
        if (!$eliminarEnDB) {
            die("Falló la consulta");
        }
        $_SESSION['message'] = "Se eliminó definitivamente la categoría.";
