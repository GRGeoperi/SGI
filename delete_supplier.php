...
if ($datosRecibidos) {
    ...
    if ($permiso > 0) {
        $idAEliminar = $_GET['id_proveedor'];
        $eliminarFila = "DELETE FROM proveedor WHERE id_proveedor = $idAEliminar";
        $eliminarEnDB = mysqli_query($conexion, $eliminarFila);
        if (!$eliminarEnDB) {
            die("Falló la consulta");
        }
        $_SESSION['message'] = "Se eliminó definitivamente el proveedor.";
